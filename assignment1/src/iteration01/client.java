...
public class client {
	...
	private DatagramPacket createReceivePacket(byte[] b) {
		return new DatagramPacket(b, b.length);
	}
	
	private DatagramPacket createSendPacket(byte[] rq) {
		DatagramPacket dp = null;
		try {
			dp = new DatagramPacket(rq, rq.length, InetAddress.getLocalHost(), 23);
		} catch (UnknownHostException e){
			e.printStackTrace();
			System.exit(1);
		}
	...
	// Exit from client gracefully (no need for CTRL + C)
